...
public class BinarySearchAutocomplete implements Autocompletor {
	 ...
	 * 
	 * @param terms
	 *            - A list of words from which to form terms
	 * @param weights
	 ...
	 * 
	 * @param prefix
	 *            - A prefix with which all returned words must start
	 * @param k
	...
	public List<Term> topMatches(String prefix, int k) {
		...
		Term termy = new Term(prefix, 0);
		
		if (k == 0) return list;
		
		//find matches with same prefix
		Comparator<Term> comp = new Term.PrefixOrder(prefix.length());
		int indexFirst = firstIndexOf(myTerms, termy, comp);
		int indexLast = lastIndexOf(myTerms, termy, comp);
		if (indexFirst != -1 && indexLast != -1) {
			while (indexFirst != indexLast) {
				list.add(myTerms[indexFirst]);
				indexFirst += 1;
			}
		}
